...
HisPossibleToGoObjectif FindArray_s::_IsGoInNotSafeZone(const ManageIa& ManageMap, const Poss_t& Poss, size_t x, size_t y, size_t& count, const PassIaInfoGame_t& GameInfo, const TInveotry& Inv)
{
    // /* regarde en haut */
    auto TmpType = static_cast<IAMapElemType>(this->_Array[y - 1][x]);
    HisPossibleToGoObjectif RetHistory;

    if (TmpType != IAMapElemType::UNBREAKABLE and TmpType != IAMapElemType::ME and this->_Array[y - 1][x] <= 0) {
        // printf("Boucle Ddanger zone: x:%ld y:%ld\n", x, y - 1);
    ...
    if (TmpType != IAMapElemType::UNBREAKABLE and TmpType != IAMapElemType::ME and this->_Array[y][x - 1] <= 0) {
        // printf("Boucle DDanger zone: x:%ld y:%ld\n", x - 1, y);
